...
async def ban(interaction: discord.Interaction, kullanıcı: discord.Member, sebep: str):
    ...
    embed.add_field(name="Kullanıcı", value=f"{kullanıcı.mention}", inline=False)
...
async def sustur(interaction: discord.Interaction, kullanıcı: discord.Member, sure: int, sebep: str):
    ...
    try:
        await kullanıcı.send(f"**{interaction.guild.name}**\nSebep: {sebep} nedeniyle {sure} dakika susturuldunuz.")
    except:
        pass

    await interaction.response.send_message(f"✅ {kullanıcı.mention} {sure} dakika boyunca susturuldu.")
...
async def mod_log_ayarla(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.response.send_message(f"Mod log kanalı {kanal.mention} olarak ayarlandı.")
...
async def afk(interaction: discord.Interaction, sebep: str):
    ...
    try:
        ...
        pass
    await interaction.response.send_message(f"Başarıyla AFK oldunuz. Sebep: {sebep}")
...
async def kilit_ac(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.response.send_message(f"🔒 {kanal.mention} kilitlendi.")
...
async def kilit_kapat(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await kanal.set_permissions(interaction.guild.default_role, overwrite=overwrite)
    await interaction.response.send_message(f"🔓 {kanal.mention} kullanıma açıldı.")
...
async def uyari_ver(interaction: discord.Interaction, kullanıcı: discord.Member, sebep: str):
    ...
    if ayar:
        ...
        try:
            ...
            await kullanıcı.timeout(datetime.timedelta(minutes=dakika), reason="Otomatik uyarı sistemi")
        except:
            pass

    await interaction.response.send_message(f"⚠️ {kullanıcı.mention} uyarıldı. Güncel Uyarı: {seviye}\nSebep: {sebep}")

@bot.tree.command(name="uyarılar")
async def uyari_liste(interaction: discord.Interaction, kullanıcı: discord.Member):
    adet = uyarilar.get(kullanıcı.id, 0)
    await interaction.response.send_message(f"👤 {kullanıcı.mention} kullanıcısının toplam **{adet}** uyarısı var.")
...
async def uyari_ayar(interaction: discord.Interaction, 
    ...
    await interaction.response.send_message("✅ Uyarı seviyeleri ve cezaları başarıyla ayarlandı.")
...
async def anket(interaction: discord.Interaction, soru: str, c1: str, c2: str, c3: str = None):
    ...
    await interaction.response.send_message("Anket başlatılıyor...", ephemeral=True)
...
if __name__ == "__main__":
    if not TOKEN:
        print("❌ HATA: Discord Token bulunamadı! Lütfen Secrets/Environment Variables kısmını kontrol edin.")
    else:
        bot.run(TOKEN)
